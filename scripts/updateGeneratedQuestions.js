...
const questionsPath = path.join(projectRoot, 'src', 'questions', 'generated_questions.md')
...
CSS environment variables expose the safe area insets so layouts adapt automatically across devices. UA sniffing and magic numbers are brittle, and extra wrappers do not guarantee safe spacing.`
  ...
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `The design team wants a blurred glass effect behind the sticky header using \`backdrop-filter\`, but you must provide a graceful fallback for browsers without support. What should you do?

### Options
- Apply a solid fallback background, then wrap the blur declaration inside \`@supports (backdrop-filter: blur(12px))\`.
- Add vendor prefixes for every browser and assume the blur will work.
- Toggle the blur with JavaScript feature detection on every scroll.
- Switch to an SVG filter so all browsers behave the same.

### Correct Answers
- [0] Apply a solid fallback background, then wrap the blur declaration inside \`@supports (backdrop-filter: blur(12px))\`.

### Explanation
\`@supports\` allows you to layer progressive enhancements while keeping a reliable fallback. Prefixes do not cover all engines, and SVG filters impose additional complexity.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `A responsive chart must never shrink below 320px, but should cap at half the viewport width on large displays. Which CSS expression satisfies both constraints?
...
### Correct Answers
- [0] \`width: clamp(320px, 50vw, 50vw);\`

### Explanation
\`clamp()\` enforces a minimum and maximum while using the preferred value (here, 50vw). The other expressions either ignore the minimum or produce incorrect values on different viewport sizes.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `A KPI badge animates from 0 to the live visitor count using \`setInterval\` with a 5ms delay, causing dropped frames. How should you smooth the animation?

### Options
- Rework the animation to run inside \`requestAnimationFrame\`, calculating the next value based on elapsed time per frame.
- Increase the interval to 16ms and hope the browser keeps up.
- Convert the badge to a CSS keyframe animation tied to an arbitrary duration.
- Update the count once with \`setTimeout\` instead of animating it.

### Correct Answers
- [0] Rework the animation to run inside \`requestAnimationFrame\`, calculating the next value based on elapsed time per frame.

### Explanation
\`requestAnimationFrame\` syncs updates with the browser's refresh rate, enabling smooth animation that adapts to the device. Simply adjusting timers or switching to CSS without dynamic data does not guarantee smooth updates.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `A layout needs to display messages like “Updated 5 minutes ago” in the visitor’s locale without bundling a large library. Which API should you use?

### Options
- \`Intl.RelativeTimeFormat\` to format relative timestamps based on locale data.
- \`Date.prototype.toLocaleDateString()\` with custom string concatenation.
- Include Moment.js to handle the formatting.
- Build a lookup table of translations manually.

### Correct Answers
- [0] \`Intl.RelativeTimeFormat\` to format relative timestamps based on locale data.

### Explanation
\`Intl.RelativeTimeFormat\` provides localized relative time strings out of the box. \`toLocaleDateString()\` handles absolute dates only, and third-party libraries add unnecessary weight.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `An infinite comment thread inserts new replies via Ajax. You need every “Like” button to work without rebinding listeners after each response. What should you do?

### Options
- Attach a single click listener to the comment list container and act when \`event.target.matches('.js-like-button')\`.
- Rebind click handlers to every like button after each Ajax response.
- Add inline \`onclick\` attributes to the buttons when rendering HTML.
- Wrap each reply in a <form> that submits via GET.

### Correct Answers
- [0] Attach a single click listener to the comment list container and act when \`event.target.matches('.js-like-button')\`.

### Explanation
Event delegation on a stable ancestor handles existing and future buttons with one listener. Rebinding or inline handlers scale poorly, and wrapping buttons in forms changes semantics.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `Editor dashboards list dozens of preview cards, making initial paint sluggish. You want the browser to skip rendering cards until they scroll into view without removing them from the DOM. Which CSS feature helps?

### Options
- Apply \`content-visibility: auto\` plus an appropriate \`contain-intrinsic-size\` to each card.
- Hide off-screen cards with \`display: none\` via JavaScript.
- Set \`opacity: 0\` on cards outside the viewport.
- Wrap the list in a fixed-height container with \`overflow: auto\`.

### Correct Answers
- [0] Apply \`content-visibility: auto\` plus an appropriate \`contain-intrinsic-size\` to each card.

### Explanation
\`content-visibility: auto\` lets the browser skip layout and paint work for off-screen elements while reserving space using \`contain-intrinsic-size\`. Hiding elements manually or changing opacity still costs layout work.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `The primary call-to-action needs larger tap targets on touch devices without affecting desktop spacing. What is the best technique?

### Options
- Use \`@media (pointer: coarse)\` to increase padding and hit area for touch pointers.
- Detect mobile user agents and inject inline styles.
- Apply the larger padding to the \`:hover\` state instead.
- Duplicate the button markup for mobile and desktop.

### Correct Answers
- [0] Use \`@media (pointer: coarse)\` to increase padding and hit area for touch pointers.

### Explanation
Pointer media queries target input characteristics like coarse pointers, letting you adjust hit areas without UA sniffing or duplicate markup. Hover styles and UA detection are unreliable.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `Your facet filters store state in a nested object. Resetting the filters mutates the default object because it shares references with the working copy. How can you create a reliable deep clone without adding a dependency?

### Options
- Use \`structuredClone(defaultFilters)\` to create an independent copy.
- Spread the object with \`{ ...defaultFilters }\`.
- Serialize and parse with \`JSON.parse(JSON.stringify(defaultFilters))\`.
- Call \`Object.freeze(defaultFilters)\` before copying.

### Correct Answers
- [0] Use \`structuredClone(defaultFilters)\` to create an independent copy.

### Explanation
\`structuredClone\` performs a structured deep clone, preserving types like Maps and Dates. Shallow spreads leave nested references intact, and JSON stringify fails on complex types.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `The hero background video should load only for users who are not on data-saver connections. What client-side check should you add?

### Options
- Inspect \`navigator.connection?.saveData\`` and skip loading the video when it is \`true\`.
- Always preload the video; the browser will automatically throttle downloads.
- Store a cookie the first time the user visits and reuse that value.
- Autoplay the video muted so it consumes less data.

### Correct Answers
- [0] Inspect \`navigator.connection?.saveData\`` and skip loading the video when it is \`true\`.

### Explanation
The Network Information API exposes the user's data-saver preference, letting the site skip large assets when save-data is enabled. Relying on muted autoplay or cookies does not respect current connection preferences.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `Rendering a GeoJSON heatmap requires merging a 5MB dataset before painting markers, freezing the UI for two seconds. How can you keep the interface responsive?

### Options
- Move the transformation into a Web Worker and post the processed data back to the main thread.
- Wrap the processing in \`setTimeout\` so it runs after rendering.
- Increase the frame rate by calling \`requestAnimationFrame\` inside the loop.
- Turn on hardware acceleration with CSS transforms.

### Correct Answers
- [0] Move the transformation into a Web Worker and post the processed data back to the main thread.

### Explanation
Web Workers run in a separate thread, preventing heavy computation from blocking the UI. Timers and animation calls still execute on the main thread and won't eliminate jank.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `A hero section uses a CSS background image and must look crisp on high-density displays without shipping two stylesheets. What should you do?

### Options
- Use \`background-image: image-set(url('hero@1x.jpg') 1x, url('hero@2x.jpg') 2x);\`.
- Swap the background for an \`<img>\` tag with \`srcset\`.
- Load the retina image for everyone to keep things simple.
- Add JavaScript to detect device pixel ratio and change the background URL.

### Correct Answers
- [0] Use \`background-image: image-set(url('hero@1x.jpg') 1x, url('hero@2x.jpg') 2x);\`.

### Explanation
\`image-set()\` lets the browser choose the appropriate asset for the device pixel ratio while keeping background semantics. Serving the high-res image everywhere wastes bandwidth, and scripting increases complexity.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `The marketing layout must support both left-to-right and right-to-left languages without duplicating rule sets. Which CSS approach helps most?

### Options
- Replace physical properties like \`margin-left\` with logical properties such as \`margin-inline-start\` and \`margin-inline-end\`.
- Create a separate stylesheet for RTL locales and swap it server-side.
- Add \`[dir="rtl"]\` classes to every component and override styles manually.
- Rely on automatic mirroring from the browser without changing CSS.

### Correct Answers
- [0] Replace physical properties like \`margin-left\` with logical properties such as \`margin-inline-start\` and \`margin-inline-end\`.

### Explanation
Logical properties adapt automatically based on document direction, reducing duplication. Separate stylesheets or manual overrides increase maintenance and risk inconsistencies.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `Your component library mixes resets, utilities, and component styles. Conflicting specificity makes it hard to predict overrides. Which modern CSS feature helps organize the cascade?

### Options
- Group styles with \`@layer\` (e.g., \`@layer reset, utilities, components\`) and load files in that order.
- Add \`!important\` to the component rules.
- Inline critical CSS at the top of every template.
- Split styles into multiple stylesheets and hope they load alphabetically.

### Correct Answers
- [0] Group styles with \`@layer\` (e.g., \`@layer reset, utilities, components\`) and load files in that order.

### Explanation
\`@layer\` defines cascade layers, letting you control precedence without relying on selector specificity battles. Heavy use of \`!important\` or arbitrary file splits leads to brittleness.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `Product managers want to know whether the hero image delays Largest Contentful Paint on key landing pages. Which browser API provides real-time LCP measurements?

### Options
- Use \`PerformanceObserver\` listening for entries of type \`'largest-contentful-paint'\`.
- Inspect the \`window.performance.timing\` object manually.
- Trigger Chrome DevTools remotely for each page load.
- Estimate LCP based on DOMContentLoaded time.

### Correct Answers
- [0] Use \`PerformanceObserver\` listening for entries of type \`'largest-contentful-paint'\`.

### Explanation
\`PerformanceObserver\` captures paint metrics like LCP during runtime. Legacy timing APIs lack this detail, and manual DevTools inspection cannot run on every user session.`
  },
  {
    domain: DOMAINS.FUNDAMENTAL,
    content: `Analytics must record a final engagement event even when users close the tab immediately after clicking an outbound link. What is the most reliable browser API?

### Options
- Call \`navigator.sendBeacon()\` with the payload before navigating away.
- Use a synchronous XMLHttpRequest inside \`beforeunload\`.
- Delay navigation with \`event.preventDefault()\` and \`setTimeout\`.
- Rely on the analytics script to retry later.

### Correct Answers
- [0] Call \`navigator.sendBeacon()\` with the payload before navigating away.

### Explanation
\`sendBeacon()\` queues data to send asynchronously, even during unload, without blocking navigation. Synchronous requests freeze the UI, and delaying navigation hurts user experience.`
  }
]

const themingQuestions = [
  {
...
function getDomainFromRange(id) {
  const match = EXISTING_DOMAIN_RANGES.find(range => id >= range.start && id <= range.end)
  if (!match) {
    throw new Error(`Unable to infer domain for question ${id}`)
  }
  return match.domain
}

function parseExistingQuestions(markdown) {
...
function buildMarkdown(questions) {
  return questions
    .map((question, index) => {
      ...
      const body = trimmedContent.length > 0 ? `${trimmedContent}\n\n` : ''
...
function sortQuestions(questions) {
  const domainIndex = domain => {
    const idx = DOMAIN_ORDER.indexOf(domain)
    return idx === -1 ? DOMAIN_ORDER.length : idx
  }

  return [...questions].sort((a, b) => {
    const domainDiff = domainIndex(a.domain) - domainIndex(b.domain)
...
function countByDomain(questions) {
  ...
  for (const question of questions) {
    if (!counts.hasOwnProperty(question.domain)) {
      counts[question.domain] = 0
    }
...
function main() {
  ...
  const existingQuestions = parseExistingQuestions(raw)
  ...
  const additionalWithOrder = additionalQuestions.map((question, index) => ({
    ...question,
    originalOrder: existingQuestions.length + index
  }))

  const combined = sortQuestions([...existingQuestions, ...additionalWithOrder])
  const markdown = buildMarkdown(combined)

  fs.writeFileSync(questionsPath, markdown, 'utf8')

  const counts = countByDomain(combined)
  console.log('Updated question counts by domain:')
  for (const domain of DOMAIN_ORDER) {
    console.log(`${domain}: ${counts[domain] || 0}`)
  }
